            ...
            if "detail" in error_data:
                ...
                if isinstance(detail, list):
                    ...
                    for error_item in detail:
                        if isinstance(error_item, dict) and "loc" in error_item and "msg" in error_item:
                            field = ".".join([str(loc_part) for loc_part in error_item["loc"] if loc_part != "body"])
                            ...
                            if "password" in field:
                                ...
                                elif "at least one uppercase letter" in error_item["msg"]:
                                    error_messages.append("Password must contain at least one uppercase letter")
                                elif "at least one lowercase letter" in error_item["msg"]:
                                    error_messages.append("Password must contain at least one lowercase letter")
                                elif "at least 8 characters" in error_item["msg"] or "shorter than" in error_item["msg"]:
                                    error_messages.append("Password must be at least 8 characters long")
                                else:
                                    error_messages.append(f"Password: {error_item['msg']}")
                            elif "email" in field:
...
async def api_request(
    ...
    try:
        ...
        if DEBUG_MODE:
            ...
            if "Authorization" in safe_headers:
                ...
                if len(auth_token) > 15:
                    safe_headers["Authorization"] = auth_token[:10] + "..." + auth_token[-5:]
        ...
        if DEBUG_MODE and "/auth/register" in path:
            logging.error(f"Registration error: {e.response.status_code} - {e.response.text}")
        handle_api_error(e)
        return None
